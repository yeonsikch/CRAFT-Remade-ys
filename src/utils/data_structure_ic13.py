	...
	for image_name in all_transcriptions:
		...
		with open(base_path + '/' + i, 'r') as f:
			...
			for no,line in enumerate(lines):
				...
				minX,minY,maxX,maxY = line[:4]
				annots = [minX,minY,maxX,minY,maxX,maxY,minX,maxY]
				text = line[-1]
				cur_annot.append(np.array(annots, dtype=np.int32).reshape([4, 2]).tolist())
				cur_text.append(text)
			all_annots['annots'][image_name]['bbox'] = cur_annot
...
if __name__ == "__main__":
	...
	# icdar2013_test()
	icdar2013_train('/data2/yeonsik/std/ICDAR13/train', '/data2/yeonsik/std/ICDAR13/train_gt', 'new_train_gt.json')
