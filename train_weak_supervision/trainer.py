...
def train(model, optimizer, iteration):
	...
	test_f1, test_pre, test_rec = 0,0,0
	...
	for no, (
		...
		if np.array(all_count)[-min(100, len(all_count)):].sum() != 0:
			...
			recall = 0
#
		loss_ = int(loss.item() * config.optimizer_iterations * 100000) / 100000
		iterator.set_description(
			'Loss:' + str(loss_) + ' Iterations:[' + str(no)
			+ '/' + str(len(iterator)) +
			'] Average Loss:' + str(
		...
		if (no + 1) % config.test_now == 0:
			...
			test_f1, test_pre, test_rec = test(model, iteration)
			print('\nF-score of testing: ', test_f1, test_pre, test_rec, '\n')
			wandb.log({
				'Iter' : iteration,
				'no' : no,
				'Test F-score' : test_f1,
		...
		wandb.log({
			'Iter' : iteration,
			'no' : no,
			'lr' : config.lr[iteration]*(0.8**((no + 1)//config.change_lr - 1)),
			'Train Average Loss' : int(np.array(all_loss)[-min(1000, len(all_loss)):].mean() * 100000) / 100000,
			'Train Average F-Score' : f_score,
			'Train Average Recall' : recall,
			'Train Average Precision' : precision,
			'Test F-score' : test_f1,
			'Test Precision' : test_pre,
			'Test Recall' : test_rec,
		})
